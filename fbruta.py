    ...
    def percorre_grafo(self, v, ncaminho):
        ...
        if not self.terminou(v, self.caminhos[ncaminho - 1].arestas_percorridas):
            ...
            # senao, vamos verificar qual aresta é melhor
                ...
                for idx, vizinho in enumerate(self.vertices[v].vizinhos):
                    print(idx)
                    print(self.vertices[v].partida)
                    if not self.caminho_visitado(vizinho, self.caminhos[ncaminho - 1].arestas_percorridas) or self.vertices[vizinho.destino].partida:
                        self.avanca_proximo(v, vizinho, ncaminho + idx)
                    else:
                        continue
